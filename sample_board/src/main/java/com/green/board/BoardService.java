...
//		
////		제목 중복체크
//		boolean isTitle = boardmapper.isTitle(bdto.getTitle());
//		
////		중복체크
//		if(isTitle == false) {
...
//		int result = boarddao.insertBoard(bdto);
		return boardmapper.allSelectBoard();
	}
...
//	게시물 상세 페이지
	public BoardDTO oneBoardDe(int id) {
		...
		return boardmapper.oneBoard(id);
	}
...
//	게시물 삭제
	public boolean delOneBoard(int id) {
		...
		return boardmapper.delBoard(id) == 1;
	}
...
//	게시물 검색 메소드
	public List<BoardDTO> SearchBoard(String shearchKeyWord, String searchType) {
		...
		return boardmapper.getSearch(shearchKeyWord, searchType);
	}
}
